"""AI 图片生成工具 (讯飞 HiDream) — prompt 精炼由 image service 统一处理"""
...
async def generate_image(prompt: str, user_id: str, aspect_ratio: str = "1:1", img_count: int = 1, chat_group_id: str = "0"):
    ...
    参数：prompt图片描述，user_id用户数字ID，aspect_ratio宽高比(1:1/16:9/9:16等)，img_count生成数量(1-4)，chat_group_id聊天组ID"""
    from backend.src.service.image import service as image_service

    gid = int(chat_group_id) if str(chat_group_id).isdigit() else 0
    try:
        records = await image_service.generate(prompt, user_id, aspect_ratio, img_count, chat_group_id=gid)
