...
async def list_learning_paths(user_id: str):
    ...
    lines = ["可用的学习路径："]
    for p in paths:
        lines.append(f"- [ID:{p['path_id']}] {p['subject']}（{p['difficulty']}，{p['node_count']} 节点）")
    return "\n".join(lines)
...
async def get_learning_path_detail(path_id: int, user_id: str):
    ...
    for n in nodes:
        rtypes = ", ".join(n.get("resource_types", []))
        quiz = n.get("quiz_config", {})
        quiz_str = f"测验 {quiz.get('count', 3)}题/通过线{quiz.get('threshold', 0.7)}" if quiz else "无测验"
        lines.append(f"  {n['order_index']}. {n['topic']} — 资源：{rtypes or '文档'} — {quiz_str}")
    return "\n".join(lines)
...
async def enroll_learning_path(path_id: int, user_id: str):
    ...
    try:
        ...
        first = next((n for n in nodes if n["status"] == "unlocked"), None)
...
@tool
async def update_path_node(path_id: int, node_id: int, user_id: str,
                           topic: str = "", knowledge_tags: str = "",
                           resource_types: str = "", quiz_count: int = 0,
    ...
    if topic:
        fields["topic"] = topic
    if knowledge_tags:
        fields["knowledge_tags"] = [t.strip() for t in knowledge_tags.split(",") if t.strip()]
    if resource_types:
        fields["resource_types"] = [t.strip() for t in resource_types.split(",") if t.strip()]
    quiz_config = {}
    if quiz_count > 0:
...
@tool
async def add_path_node(path_id: int, topic: str, user_id: str,
                        knowledge_tags: str = "", resource_types: str = "document"):
