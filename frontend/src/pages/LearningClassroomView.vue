    ...
    <section class="classroom-layout">
      ...
      <section v-else class="lesson-workspace lesson-generation-state" aria-live="polite">
        ...
        <div v-else-if="classroomLoading" class="classroom-waiting-panel">
          ...
          <div class="waiting-progress" aria-hidden="true"><i v-for="(_, index) in transitionRotationLength" :key="index" :class="{ active: index === transitionSlideIndex }"></i><span></span></div>
          <div v-if="transitionLoading" class="waiting-skeletons"><i></i><i></i><i></i></div>
          <div v-else class="waiting-dual-grid">
            <section class="waiting-column waiting-column--news">
              <div class="waiting-section-heading"><Newspaper :size="18" /><span>近日大事</span></div>
              <Transition name="waiting-slide" mode="out-in">
                <article :key="activeTransitionNews.key" class="waiting-slide">
                  <h3>{{ activeTransitionNews.title }}</h3>
                  <p>{{ activeTransitionNews.content }}</p>
                  <a v-if="activeTransitionNews.url" :href="activeTransitionNews.url" target="_blank" rel="noopener noreferrer" class="waiting-source-link">
                    {{ activeTransitionNews.meta || '打开公开来源' }} <ExternalLink :size="14" />
                  </a>
                  <span v-else class="waiting-slide-meta">{{ activeTransitionNews.meta }}</span>
                </article>
              </Transition>
            </section>
            <section class="waiting-column waiting-column--story">
              <div class="waiting-section-heading"><Lightbulb :size="18" /><span>画像小故事</span></div>
              <Transition name="waiting-slide" mode="out-in">
                <article :key="activeTransitionStory.key" class="waiting-slide">
                  <h3>{{ activeTransitionStory.title }}</h3>
                  <p>{{ activeTransitionStory.content }}</p>
                  <span class="waiting-slide-meta">{{ activeTransitionStory.meta }}</span>
                </article>
              </Transition>
            </section>
          </div>
          <footer class="waiting-rotation-status"><span>第 {{ transitionSlideIndex + 1 }} / {{ transitionRotationLength }} 组</span><span>10 秒后同步切换</span></footer>
        </div>
...
const transitionStories = computed(() => {
  const stories = Array.isArray(transitionContent.value.stories) ? transitionContent.value.stories : []
  return stories.length ? stories.map((item, index) => ({
    key: `story-${index}`,
    title: String(item?.title || '一个小问题的转弯'),
...
const resourceIcon = resource => {
  const kind = classroomResourceKind(resource)
  if (kind === 'ppt') return Presentation
  if (kind === 'mindmap') return GitBranch
  return FileText
}
...
const startTransitionRotation = () => {
  ...
  if (!classroomLoading.value || transitionRotationLength.value < 2) return
  transitionRotationTimer = window.setInterval(() => {
    transitionSlideIndex.value = (transitionSlideIndex.value + 1) % transitionRotationLength.value
  }, 10000)
}
...
.resource-workbench{padding:14px 14px 12px}.resource-workbench-intro{margin:6px 0 10px;color:#6383a4;font-size:11px;line-height:1.4}.resource-items{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;max-height:none;overflow:visible}.resource-item{display:grid;place-items:center;gap:8px;min-width:0;min-height:88px;padding:10px 8px;border:1px solid #dcebf6;border-radius:10px;background:#fff;cursor:pointer;transition:transform .14s ease,border-color .14s ease,box-shadow .14s ease,background .14s ease}.resource-item:hover:not(:disabled){border-color:#b7d3ec;background:#f8fbff;box-shadow:0 8px 18px rgba(38,93,150,.08);transform:translateY(-1px)}.resource-item:disabled{cursor:wait;opacity:.62}.resource-item-icon{display:grid;place-items:center;width:34px;height:34px;border-radius:10px;background:#edf5ff;color:#2466a9}.resource-item--ppt .resource-item-icon{background:#edf7ff;color:#1f68ae}.resource-item--mindmap .resource-item-icon{background:#eefaf4;color:#2d8a61}.resource-item--document .resource-item-icon{background:#f3f7fc;color:#456892}.resource-item strong{min-width:0;overflow:hidden;color:#174b91;font-size:12px;line-height:1.2;text-align:center;text-overflow:ellipsis;white-space:nowrap}
...
@media(max-width:680px){.classroom-page{gap:10px;padding:12px}.classroom-header{flex-wrap:wrap;gap:10px}.back-button span{display:none}.back-button{width:42px;justify-content:center;padding:0}.classroom-heading{flex:1 1 calc(100% - 60px)}.classroom-heading h1{font-size:22px}.generation-status{order:3;width:100%;justify-content:center;margin-left:0}.module-nav{grid-template-columns:repeat(4,minmax(78px,1fr));overflow-x:auto}.module-tab{min-height:48px;padding:7px}.module-tab-copy small{display:none}.module-panel-header{padding:18px 16px 14px}.module-panel-header h2{font-size:26px}.module-content{grid-template-columns:1fr;padding:14px 16px}.speech-block{grid-row:auto;min-height:0}.question-block{grid-column:auto}.module-footer{padding:12px 16px 16px}.secondary-button,.primary-button{flex:1;padding:0 10px;font-size:13px}.resource-items{grid-template-columns:repeat(3,minmax(0,1fr));gap:6px}.resource-item{min-height:80px;padding:9px 6px}.resource-item-icon{width:32px;height:32px}.resource-item strong{font-size:11px}.resource-preview-overlay{padding:10px}.resource-preview-panel{height:calc(100vh - 20px);border-radius:12px}.resource-preview-panel header,.resource-preview-panel footer{padding:12px}.resource-preview-panel header h2{max-width:250px;font-size:17px}.resource-preview-body{padding:12px}}
